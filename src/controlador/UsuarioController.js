...
export class UsuarioControlador {
    static async postGuardarUsuarios(req, res) {
      ...
      let resultado = await UsuarioModelo.registrarNuevoUsuario(nombre, correo, claveEncriptada, validarUsuario);
      if (resultado) {
    ...
    static async cambiarClaveUsuario(req, res) {
      ...
      if (claveVieja || claveNueva || claveNuevaConfirmar) {
        if (claveNueva.length < 5 || claveNueva.length > 16 && claveNuevaConfirmar.length < 5 || claveNuevaConfirmar.length > 16) {
          return res.status(400).send({
            status: "Error",
            message: "Entre 5 y 16 caracteres",
          });
        ...
        if (claveNueva === claveNuevaConfirmar) {
          ...
          if (comparada) {
            let resultadoCambiarClave = await UsuarioModelo.claveActualizadaUsuario(claveEncriptada, correo);
